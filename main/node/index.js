...
const fsPlus = {
    ...
    getSizeSync(pathToCheck) {
        if (isPathValid(pathToCheck)) {
            let size = statSyncNoException(pathToCheck).size
    ...
    allFilesRecursivelySync(directoryName) {
        ...
        for (let each of files) {
            ...
            // explore all sub directories
            if (fileStats.isDirectory()) {
                allFiles = allFiles.concat(fsPlus.allFilesRecursivelySync(fullPath))
            }
        }
        return allFiles
    },
    ...
    sync: {
        ...
        listRecursive(...args) {
            return fsPlus.allFilesRecursivelySync(...args)
        },
